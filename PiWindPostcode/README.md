...
# PiWind Postcode
Toy UK windstorm and storm surge model for exposures input at postcode resolution.
...
An example pre-analysis adjustment logic is available to demonstrate partial disaggregation for aggregate risks with unknown occupancy.
...
In order to avoid large aggregate portfolios being fully disaggregated pre-analysis (which can produce very large location files and slow down file preparation runtimes) Oasis has a feature which disaggregates locations with identical risk characteristics and multiple buildings into individual buildings for the purposes of ground up loss sampling in the kernel.  
